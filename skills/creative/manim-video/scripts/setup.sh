...
command -v pdflatex &>/dev/null && ok "LaTeX (pdflatex)" || { fail "LaTeX not found (macOS: brew install --cask mactex-no-gui; FreeBSD: review native TeX packages providing pdflatex, latex and dvisvgm before an approved install)"; errors=$((errors+1)); }
